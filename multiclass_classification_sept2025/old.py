...
current_date = f"{datetime.datetime.now().day}-{datetime.datetime.now().month}"  # TODO: this is bad because there are cases when training lasts from 23:00 to 3:00 or sth like that..
...
for epoch in range(START_EPOCH, EPOCHS):
    ...
    multiclass_metrics = compute_multiclass_metrics(model, test_dataset, device)
    logger.info(f"Multiclass Test F1 for epoch {epoch}: {multiclass_metrics['f1_score']:.4f}")
    print("TEST")
    ...
    if (epoch % SAVE_MODEL_EVERY_N_EPOCHS == 0 and epoch > 0) or epoch == EPOCHS - 1:
        ...
        metrics = {
            ...
            'test_pr_auc': test_result['average_precision'],
...
# plot_and_save_curves(metrics, EPOCHS - 1, dataset_name, save_dir=f"{plot_save_curves_dir}")  # Финальные графики без epoch_label